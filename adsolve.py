    ...
    def diff_tangent(self, dependees_diff_u):
        if hasattr(self, 'residual') and self.residual:
            ...
            if resid_diff_u is 0:
                ...
                try:
                    is_diag = all(resid_diff_self.indices == arange(n)) and \
                              all(resid_diff_self.indptr == arange(n+1))
                except TypeError:
                    is_diag = False
                if is_diag:
                    # inverse of diagonal matrix
...
def solve(func, u0, args=(), kargs={},
          max_iter=10, abs_tol=1E-6, rel_tol=1E-6, verbose=True, lam=1.):
    ...
    for i_Newton in range(max_iter):
        ...
        try:
            ...
            res_norm = np.linalg.norm(np.array([res._value]),
                                      np.inf)
        if verbose:
        ...
        u._value -= lam * minus_du.reshape(u.shape)
