...
def objective(w_interior, geo, coef, xin, xout, yin, yout):
    ...
    outflux = sum( w[1][-1,:] * np.diff(geo.xy[1][-1,:]) )    # correct
    # outflux = sum( w[1][-1,:] * (geo.xy[1][-1,1:]-geo.xy[1][-1,:-1]) )  # also right, but different
    giny = asarray(outflux.diff(yin))[0]
    print(sum(giny[pert_section])*1e-4)
...
if __name__ == '__main__':
    ...
    # generate base normal direction
    geo = geo2d([x,y])
    normal = geo.dxy_i[:,pert_section,0]._value
    L = np.sqrt(normal[0]**2 + normal[1]**2)
    normal[0] /= L
    ...
    coef = array([1.,1.])
    var_grad(coef, x, y, xin, yin, xout, yout)
    set_trace()    
