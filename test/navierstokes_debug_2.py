...
def visual_compare(w, geo, state):
    # Visualize Mach number, non-dimensionalized stagnation and static pressure
...
def primative(w, state):
    ...
    v = w[2] / rho
    E = w[3]
    p = state.P( E - .5*(u*w[1]+v*w[2]), rho)[0]
    return rho, u, v, E, p
...
def flowsolver(xy, state):
    ...
    w0 = loadtxt('sol/standard_w')
    state._last_inlet_U = loadtxt('sol/inlet_U')
    state._last_outlet_U = loadtxt('sol/outlet_U')

    dt = np.inf
    w = solve(ns_kec, w0, args=(w0, geo, dt, state), rel_tol=1E-5, abs_tol=1E-4)
    w0.obliviate()
    w0 = w
        ...
        for i in range(self.Un):
            for j in range(self.Rn):
                result += sigmoid(self.Ubeta*(U-self.Uis[i])) * \
                          ...
                          self.coef[i,j]
        result += 0.04*U + 1e5*R
        return result, result.diff(self.coef)

    def _P_res_(self, U, P, R):
        return self.P(U, R)[0] - P
...
def mismatch(w, state):
    w = w.reshape([4, Ni, Nj])
    rho, u, v, E = primative(w, state)[:-1]
    ...
    try:
        grad = asarray( err.diff(state.coef) )[0]
    except IndexError:
        grad = asarray( err_rho.diff(state.coef).todense() )[0]
    ...
    if not any(isnan(grad)):
        ...
        return 1e10, 1e9 * (state.coef - last_valid_state.coef)._value
...
if __name__ == '__main__':
    ...
    state = StateEqn(5,5,[1.5e5,4.5e5],[0.4,1])
    state.setcoef(np.ones([5,5])*7e3)    # perturbation +10.
    w = flowsolver(array([x,y]), state)
    set_trace()
    

    '''
    # use base geometry to train coef
