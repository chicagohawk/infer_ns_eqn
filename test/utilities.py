...
def visualize(w, geo, coef):
    ...
    avgM_s     = np.loadtxt('sol/mike/vis_avgM')
    u_s        = np.loadtxt('sol/mike/vis_u')
    v_s        = np.loadtxt('sol/mike/vis_v')
    avgpt_s    = np.loadtxt('sol/mike/vis_avgp')
    avgp_s     = np.loadtxt('sol/mike/vis_avgpt')
    ...
    contourf(x, y, degrade(avg(pt_frac))-avgpt_s, 100)
    colorbar()
    axis('scaled')
    xlabel('x')
    ylabel('y')
    ...
    contourf(x, y, degrade(avg(p_frac))-avgp_s, 100)
    colorbar()
    axis('scaled')
    xlabel('x')
    ylabel('y')
...
# ---------------------- objective ---------------------------- #
def objective(w_interior, geo, coef, xin, xout, yin, yout):
    # total outlet mass flux
    w = w_interior.reshape([4, Ni, Nj])
    outflux = sum( w[1][-1,:] * np.diff(geo.xy[1][-1,:] ) )
    gradin, gradout = None, None

    # in boundary gradient
    ginx = asarray(outflux.diff(xin))[0]
    giny = asarray(outflux.diff(yin))[0]

    ginx = (ginx[:-1]+ginx[1:])/2.
    giny = (giny[:-1]+giny[1:])/2.
    
    gradin = ginx * geo.normal_j[0,:,0] + giny * geo.normal_j[1,:,0]
    gradin[geo.normal_j[0,:,0]._value==0] = 0.
    
    # out boundary gradient
    goutx = asarray(outflux.diff(xout))[0]
    gouty = asarray(outflux.diff(yout))[0]
    goutx = (goutx[:-1]+goutx[1:])/2.
    gouty = (gouty[:-1]+gouty[1:])/2.
    
    gradout = goutx * geo.normal_j[0,:,-1] + gouty * geo.normal_j[1,:,-1]
    gradout[geo.normal_j[0,:,-1]._value==0] = 0.
...
def var_grad(coef, x, y, xin, yin, xout, yout):
    f = open('sol/mike/coef','a')
    f.write(array_str(degrade(coef))[1:-1]+'\n')
    f.close()
    ...
    # print('Final, t = inf')
    dt = np.inf
    w = solve(ns_kec, w0, args=(w0, geo, dt, coef), rel_tol=1E-7, abs_tol=1E-5)
    outflux, gradin, gradout = objective(w, geo, coef, xin, xout, yin, yout)

    figure(figsize=(30,10))
    visualize(w, geo, coef)
    global iter_count
    savefig('sol/mike/iter'+str(iter_count))
    iter_count += 1
    close()
    show(block=True)
...
if __name__ == '__main__':
    ...
    # clean coef history
    f = open('sol/mike/coef','w')
    f.write('')
    f.close()
    # coef = degrade( array([1.0378804, 2.7445768]) )
    coef = degrade( array([1., 1.]) )
    options = {'maxiter':50, 'disp':True, 'gtol':0}
    bnds = ((0.1, 3),(0.1,3))
    coef = minimize(var_grad, coef, args=(x,y,xin,yin,xout,yout), method='SLSQP', bounds=bnds, jac=True,)
    set_trace()
