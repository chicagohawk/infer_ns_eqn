...
def extend(w_interior, geo):
    ...
    w[3,0,1:-1] = 1/.4 * (p+1e4*rho**2) * (1-.1*rho) + .5 * rho * (u**2+v**2)
...
def primative(w):
    ...
    p = .4 * (E-.5*rho*(u**2+v**2)) / (1-.1*rho) - 1e4*rho**2
    return rho, u, v, E, p
...
def flowsolver(xy):
    ...
    for i in range(100):
        ...
        w0.obliviate()
    dt = np.inf
    w = solve(ns_kec, w0, args=(w0, geo, dt), rel_tol=1E-6, abs_tol=1E-6)
    return w
...
def objective(w, geo_base, xyin, xyout):
    # expand boundary xy
    xin, yin = xyin[0], xyin[1]
    xout, yout = xyout[0], xyout[1]
...
if __name__ == '__main__':
    ...
    w = flowsolver(array([x,y]))
    geo = geo2d(array([x,y]))
    rho, u, v, E = primative( degrade( extend(w,geo) ) )[:-1]
    set_trace()

    # plot state equation p = p(rho, U)
    U = E - .5*rho*(u**2+v**2)
    U = np.linspace(U.min(), U.max(),20)
    R = np.linspace(rho.min(),rho.max(),20)
    U,R = np.meshgrid(U,R)
    P = .4 * U / (1-.1*R) - 1e4*R**2
    contourf(U/1e4, R, P, cmap=cm.jet)

    case = 1
    PATH = '/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/gray_sol/'
    savetxt(PATH+'rho'+str(case), rho)
    savetxt(PATH+'u'+str(case),   u)
    savetxt(PATH+'v'+str(case),   v)
    savetxt(PATH+'E'+str(case),   E)
    set_trace()
