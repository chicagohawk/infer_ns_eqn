...
def flowsolver(xy, state):
    ...
    for i in range(500):
        ...
        if isinf(w._n_Newton) or isnan(w._res_norm):
            dt *= .1
            print('alert!')
            w = solve(ns_kec, w0, args=(w0, geo, dt, state), rel_tol=1E-6, abs_tol=1E-5, max_iter=20,
                      verbose=False)
            if isinf(w._n_Newton) or isnan(w._res_norm):
                return None
    ...
    dt = np.inf
    w = solve(ns_kec, w0, args=(w0, geo, dt, state), rel_tol=1E-5, abs_tol=1E-5, max_iter=40,
              verbose=False)
    ...
    def U(self, P, R, which_boundary):
        ...
        if which_boundary == 'inlet':
            if self._last_inlet_U is None:
                for relax_factor in [.75, .5, .25, .15]:
                    result = solve(self._P_res_, np.ones(degrade(R).shape)*2.5e5, 
                             args=(P,R), rel_tol=1e-7, abs_tol=1e-6, 
                             max_iter=30, verbose=False, lam=relax_factor)
                    if not isinf(result._n_Newton):
                        break
            else:
                for relax_factor in [1., .75, .5, .25]:
...
def mismatch(w_ext, state, standard_sol):
    ...
    diffs = array( [linalg.norm(rho-rho_s,4),
                    linalg.norm(u-u_s,4),
                    linalg.norm(v-v_s,4),
                    linalg.norm(E-E_s,4)] )
...
if __name__ == '__main__':
    ...
    # pertin, pertout = \
    #     normal_to_xy(.1*np.sin(np.linspace(0., np.pi, pert_section.size)), 
    #                  .1*np.sin(linspace(0., 3*np.pi, pert_section.size)),
    #                   geo_base)
    ...
    NE, NR = 10, 10
    # state = StateEqn(NE, NR,[0e5,4e5],[0.,1])     # running on left terminal 
    state = StateEqn(NE, NR,[2e5,3.5e5],[0.3,1.1])  # running on right terminal

    case = 1
    # coef = np.loadtxt('/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/'+
    #                   'train_state_eqn/coef_'+str(case)+'.txt')    # left terminal
    coef = np.ravel( 8.*np.ones([NE, NR]) )                          # right terminal
    
    bnds = tuple( map(tuple, np.tile( np.array([-25.,50.]), [coef.size,1] )) )
    DIR = '/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/gray_sol/'
