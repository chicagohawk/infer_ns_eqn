...
# ------------------------ plot geometry gradient ------------------------- #
def plot_geo_grad(gradin, gradout, geo):
    set_trace()
    pertin, pertout = normal_to_xy(gradin, gradout, geo)
    gradout_gray = loadtxt('/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/'+
                           'outflux_geo_grad/gradout_gray_0.txt')
    gradin_gray = loadtxt('/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/'+
                          'outflux_geo_grad/gradin_gray_0.txt')
    pertin_gray, pertout_gray = normal_to_xy(gradin_gray, gradout_gray, geo)

    figure(figsize=(10,16))
    # outer boundary
    gray_lg = quiver(geo.xy[0][:,-1][pert_section], geo.xy[1][:,-1][pert_section], 
                     pertout_gray[0], pertout_gray[1], width=10e-3, color='blue', 
                     scale=1/0.03, linewidth=0., zorder=10, headwidth=1.3)
    twin_lg = quiver(geo.xy[0][:,-1][pert_section], geo.xy[1][:,-1][pert_section], 
                     pertout[0], pertout[1], width=3e-3, color='r', 
                     scale=1/0.03, linewidth=0., zorder=11, headwidth=1.)
    quiverkey(gray_lg, 1.1, 1.3, 2, 'sensitivity using true (graybox) model', coordinates='data')
    quiverkey(twin_lg, 1.1, 1.15, 2, 'sensitivity using inferred model', coordinates='data')
    
    # inner boundary
    quiver(geo.xy[0][:,0][pert_section], geo.xy[1][:,0][pert_section], 
           pertin_gray[0], pertin_gray[1], width=10e-3, color='blue', 
           scale=1/0.015, linewidth=0., zorder=10, headwidth=1.3)
    quiver(geo.xy[0][:,0][pert_section], geo.xy[1][:,0][pert_section], 
           pertin[0], pertin[1], width=3e-3, color='r', 
           scale=1/0.015, linewidth=0., zorder=11, headwidth=1.)
    
    Xg = geo.xy[0][12:38]
    Yg = geo.xy[1][12:38]
    [plot(x,y,color='.75',zorder=1) for x,y in zip(Xg,Yg)]
    [plot(x,y,color='.75',zorder=1) for x,y in zip(Xg.transpose(),Yg.transpose())]
    xlabel('x')
    ylabel('y')
    
    title('sensitivity of outflux to boundary geometry')
    axis('equal')
    set_trace()
    # show(block=True)
...
if __name__ == '__main__':
    ...
    NE, NR = 10, 10
    state = StateEqn(NE, NR,[0.,4e5],[0.,1])

    case = 0
    coef = np.loadtxt('/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/'
                     +'train_state_eqn/coef_0.txt').reshape([NE,NR])

    state.setcoef(coef*1e3)
    E = np.loadtxt('/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/gray_sol/E0')
    u = np.loadtxt('/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/gray_sol/u0')
    v = np.loadtxt('/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/gray_sol/v0')
    rho = np.loadtxt('/home/voila/Documents/2014GRAD/numpad/test/ns_cases/results/gray_sol/rho0')
    U = E - .5*rho*(u**2+v**2)
    U = np.linspace(U.min(), U.max(),20)
    rho = np.linspace(rho.min(),rho.max(),20)
    U,rho = np.meshgrid(U,rho)
    P = state.P(U, rho)
    figure(figsize=(12,5))
    subplot(1,2,1)
    contourf(U/1e4,rho,P._value, levels=[9.5e4, 10e4, 10.5e4, 11e4, 11.5e4, 12e4], cmap=cm.jet)
    colorbar()
    title('pressure')
    xlabel('internal energy (1e4)')
    ylabel('density')
    subplot(1,2,2)
    contourf(U/1e4,rho,.4*U, levels=[9.5e4, 10e4, 10.5e4, 11e4, 11.5e4, 12e4], cmap=cm.jet)
    colorbar()
    title('pressure')
    xlabel('internal energy (1e4)')
    ylabel('density')
    ...
    w, geo = flowsolver(array([x,y]), state)
    outflux, gradin, gradout = objective(w, geo)

    # visualize(w, geo, state)
    visual_compare(w, geo, state, standard_sol)

    # quiver plot geometry gradient
    plot_geo_grad(gradin, gradout, geo)
    set_trace()
